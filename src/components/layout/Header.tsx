...
export default function Header() {
  ...
  return (
    <header
      role="banner"
      className="sticky top-0 z-50 w-full border-b border-neutral-200/60 bg-white/80 backdrop-blur-lg"
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav role="navigation" aria-label="Main navigation" className="hidden items-center gap-1 md:flex">
          {siteConfig.navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-md px-3 py-2 text-sm font-medium text-neutral-600 transition-colors hover:text-foreground"
            >
              {link.label}
            </Link>
          ))}
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-md p-2 text-neutral-600 hover:text-foreground md:hidden"
        >
          <svg
            className="h-6 w-6"
            fill="none"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="overflow-hidden border-t border-neutral-200/60 bg-white md:hidden"
          >
